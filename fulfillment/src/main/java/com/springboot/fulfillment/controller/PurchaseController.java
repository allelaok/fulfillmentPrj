...
public class PurchaseController {
	...
	public void addPurchase(@RequestBody PurchaseRequestDTO purchaseRequestDTO) {
		...
		PurchaseRequestDTO rePurchaseRequestDTO = purchaseService.sendPurchase(purchaseRequestDTO);

		// 응답받은 requestDTO를 통해서 풀필먼트테이블에 저장
		purchaseService.addPurchase(rePurchaseRequestDTO);
	}

	// 전달받은 업데이트발주요청(상태변화)를 서비스에 전달하여 저장
	@PutMapping
	public ResponseEntity<String> updatePurchase(@RequestBody PurchaseRequestDTO purchaseRequestDTO, @RequestHeader("sender") String sender) {
		System.out.println("===PurchaseController-updatePurchase===");
		
		// 전달받은 업데이트발주요청(상태변화)를 서비스에 전달하여 저장
		purchaseService.updatePurchase(purchaseRequestDTO);
